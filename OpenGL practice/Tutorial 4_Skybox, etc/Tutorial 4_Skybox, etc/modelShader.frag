...
void main()
{
	...
	//use reflected skybox as texture
...
void lightComponentsCalc(const LightComponents lc, const vec3 lightDir, out vec3 ambient, out vec3 diffuse, out vec3 specular){
	...
	vec3 viewDir = normalize(cameraPos - position); //from fragment to viewer's location 
...
vec3 pointLight(const PointLight light){
	...
	vec3 lightDir = light.position - position; //from object towards positional light source
